...
const loginUser = async (req, res) => {
  ...
  try {
    const query =
      "SELECT id, username, email, first_name, last_name, role, password FROM users WHERE username = $1";
...
const getUserProfile = async (req, res) => {
  ...
  try {
    ...
    const { rows } = await db.query(
      "SELECT id, username, email, first_name, last_name, dietary_preferences, role FROM users WHERE id = $1",
      [id]
    );
    if (rows.length === 0) {
      return res.status(404).json({ message: "User not found." });
...
const updateUserProfile = async (req, res) => {
  const { id } = req.params;
  const { userId, role } = req.user;
  const { username, email, first_name, last_name, dietary_preferences } =
    req.body;
...
const getAllUsers = async (req, res) => {
  ...
  if (role !== "admin") {
    return res
      .status(403)
      .json({ message: "You are not authorized to view all users." });
  }
